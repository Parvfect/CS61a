...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        Asks the insect to remove itself from the current place. This method exists so
            it can be enhanced in subclasses.
        """
        if not isinstance(insect, QueenAnt) or insect.true_queen == False:
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            ant_in_place = place.ant 
            if ant_in_place.can_contain(self):
                ant_in_place.contain_ant(self)
            elif self.can_contain(ant_in_place):
                self.contain_ant(ant_in_place)
                place.ant = self
            else:
...
class WallAnt(Ant):
    """A WallAnt protects the home base by not letting bees pass"""
    name = "Wall"
    food_cost = 4
    implemented = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def double_damage(self, ant):
        """Doubles the armour if already hasn't been"""
    ...
    def action(self, gamestate):
        ...
        if not self.true_queen:
            ...
            while pointer.exit != None:
                if pointer.ant:
                    if not pointer.ant.doubled:
                        self.double_damage(pointer.ant)
                    if isinstance(pointer.ant, ContainerAnt) and pointer.ant.contained_ant!=None:
                        self.double_damage(pointer.ant.contained_ant)
