...
def balanced(m):
    ...
    weight_left = total_weight(end(left(m)))
    weight_right = total_weight(end(right(m)))
    ...
    if length_left*weight_left == length_right*weight_right:
        return True
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        return [] + totals_tree(end(left(m))) + totals_tree(end(right(m)))
